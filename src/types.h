...
struct DirtyThreat {
    ...
    DirtyThreat(Piece pc, Piece threatened_pc, Square pc_sq, Square threatened_sq, bool add) {
        data = pc << 17 | threatened_pc << 13 | threatened_sq << 7 | pc_sq << 1 | add;
    }

    Piece  pc() const { return static_cast<Piece>(data >> 17 & 0xf); }
    Piece  threatened_pc() const { return static_cast<Piece>(data >> 13 & 0xf); }
    Square threatened_sq() const { return static_cast<Square>(data >> 7 & 0x3f); }
    Square pc_sq() const { return static_cast<Square>(data >> 1 & 0x3f); }
    bool   add() const {
        return data & 1;
    }
