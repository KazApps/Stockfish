...
Value to_corrected_static_eval(const Value v, const int cv, const Depth depth) {
    return std::clamp(v + cv / (131072 + (32 - std::min(depth, 32)) * 32), VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}
    ...
    else
    {
        ...
        if (ss->ttHit)
        {
            ...
            ss->staticEval = bestValue =
              to_corrected_static_eval(unadjustedStaticEval, correctionValue, 0);
